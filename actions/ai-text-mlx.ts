...
let mlxServerAvailable: boolean | null = null;
let lastHealthCheck = 0;
let lastMlxHealthCheck = 0;
...
// Helper function to check if the MLX API server is running
async function checkMlxServerHealth(): Promise<boolean> {
  const now = Date.now();

  // Use cached result if recent
  if (
    mlxServerAvailable !== null &&
    now - lastMlxHealthCheck < HEALTH_CHECK_INTERVAL
  ) {
    return mlxServerAvailable;
  }

  try {
    const response = await fetch(`${MLX_API_URL}/`, {
      method: "GET",
      signal: AbortSignal.timeout(2000), // 2 second timeout
    });

    mlxServerAvailable = response.ok;
    lastMlxHealthCheck = now;

    if (mlxServerAvailable) {
      console.log("✅ MLX API server is running on port 8003");
    }

    return mlxServerAvailable;
  } catch (error) {
    mlxServerAvailable = false;
    lastMlxHealthCheck = now;
    return false;
  }
}
    ...
    if (mlxAvailable) {
      try {
        const mlxResponse = await fetch(
          `${MLX_API_URL}/api/autocomplete/hybrid`,
          {
            method: "POST",
            headers: {
        ...
        if (mlxResponse.ok) {
          ...
          if (data.combined_suggestions && data.combined_suggestions.length > 0) {
            ...
            return suggestion; // MLX model already returns just the completion
          }
        }
      } catch (error: any) {
        if (error.cause?.code === "ECONNREFUSED") {
          console.error("❌ MLX API server is not running on port 8003");
          console.error("To start the server, run: ./start_mlx_server.sh");
        } else {
          console.error("MLX autocomplete API error:", error);
        }
      }
    } else {
  ...
  if (serverAvailable) {
    try {
      ...
      const hybridResponse = await fetch(
        `${PYTHON_API_URL}/api/autocomplete/hybrid`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
