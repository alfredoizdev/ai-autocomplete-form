...
echo "  ✓ HIGH-QUALITY Fine-tuned Llama-3.2-3B model"
echo "  ✓ Trained on grammar-filtered dataset (4.5k+ high-quality examples)"
echo "  ✓ Natural, grammatically correct completions"
echo "  ✓ Fast response times (100-150ms)"
echo ""
...
if lsof -Pi :8003 -sTCP:LISTEN -t >/dev/null ; then
    ...
    if lsof -Pi :8003 -sTCP:LISTEN -t >/dev/null ; then
        ...
        echo "Model should be at: models/lookingfor-llama3-3b-hq-lora/"
        echo ""
        echo "To train the model, run: ./start_training.sh"
        exit 1
    fi
fi
...
echo "  - Model: HIGH-QUALITY Llama-3.2-3B (lookingfor-llama3-3b-hq-lora)"
echo ""
echo "Next step:"
