...
const useSpellCheck = () => {
  ...
  const getSuggestions = (word: string): string[] => {
    ...
    // First, check for contraction suggestions
    const contractionSuggestions = getContractionSuggestions(cleanWord);
    
    // Then get dictionary suggestions if available
    const dictionarySuggestions = dictionary ? dictionary.suggest(cleanWord) : [];
    
    // Combine suggestions, prioritizing contractions
    const allSuggestions = [...contractionSuggestions, ...dictionarySuggestions];
